...
ProgramCounter jump_if_true(const Operation& op, ProgramMemory* memory, ProgramCounter pc) {
  int a = op.a_value(memory, pc);
  int b = op.b_value(memory, pc);
...
ProgramCounter jump_if_false(const Operation& op, ProgramMemory* memory, ProgramCounter pc) {
  int a = op.a_value(memory, pc);
  int b = op.b_value(memory, pc);
...
ProgramCounter less_than(const Operation& op, ProgramMemory* memory, ProgramCounter pc) {
  int a = op.a_value(memory, pc);
  int b = op.b_value(memory, pc);
  int rc = (*memory)[pc + 3];
...
ProgramCounter equals(const Operation& op, ProgramMemory* memory, ProgramCounter pc) {
  int a = op.a_value(memory, pc);
  int b = op.b_value(memory, pc);
  int rc = (*memory)[pc + 3];
...
int run_instruction(ProgramMemory* memory, int pc) {
  ...
  switch (op.opcode()) {
    ...
    case Opcode::JUMP_IF_TRUE:
      return jump_if_true(op, memory, pc);
    case Opcode::JUMP_IF_FALSE:
      return jump_if_false(op, memory, pc);
    case Opcode::LESS_THAN:
      return less_than(op, memory, pc);
    case Opcode::EQUALS:
      return equals(op, memory, pc);
    case Opcode::HALT:
